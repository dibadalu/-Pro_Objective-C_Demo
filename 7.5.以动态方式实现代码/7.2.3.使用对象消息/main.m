...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        ...
        NSLog(@"Invoking instance method %@ on object of class %@",NSStringFromSelector(selector3),[calc class]);
        // 然后使用该选择器调用了一个实例方法。通过动态方法决议可以在运行程序时添加和调用这个方法，并获得他的返回值
        id sum3 = [calc performSelector:selector3 withObject:sum2];
        NSLog(@"Absolute value of %@ = %@",sum2 , sum3);
        
        /*
         动态方法决议小结：
         当使用选择器absoluteValue:的消息通过performSelector:withObject:方法被以动态方式调用时，Objective-C运行时系统会将新方法(absoluteValue:)添加到Calculator类中。
         */
